...
--5 show average standard cost - rows
...
--7. amount of employees per dept - rows
--	 gender		- columns
...
select EmployeeID,Id, [eDateNotNorth], [InCommon], [both]
from  (
	  ...
	  ( select mem.Id as 'Emps_eDateNotNorthwind', mem.FirstName as 'FN_InCom', mem.LastName as 'LN_InCom'
		from edate.Operation.Members mem 
		except
...
pivot
(
	count(EmployeeID) for SourceTable in ([eDateNotNorth], [InCommon], [both])
)as pvtable;
